...
public class FractionalCalculatorTests 
{
	...
	{
		String response = ConsoleTester.getOutput("fracCalc1.FractionalCalculator", "1_1/2 + 3/4");
	...
	@Test
	public void FracCalcShouldPromptUser()
	{
		String response = ConsoleTester.getOutput("fracCalc1.FractionalCalculator", "1_1/2 + 3/4");
		System.out.println(response);
		verify.that(response).matches("Enter an expression "); // needs to be tested
		verify.that(response).matches("(or \"quit\")");  // in pieces
	...
	{
		String response = ConsoleTester.getOutput("fracCalc1.FractionalCalculator", "0 * -4_4/5");
		verify.that(response).matches("Left operand: 0[\\s\\r\\n]+Operator: \\*[\\s\\r\\n]+Right operand: -4_4/5");
	}
}